...
export default defineConfig({
  ...
  webServer: {
    command: `${path.resolve(".venv/bin/python")} -m uvicorn rdx.server:app --host 127.0.0.1 --port 8768`,
    cwd: path.resolve("."),
    url: "http://127.0.0.1:8768/api/status",
    reuseExistingServer: false,
    env: { RDX_DATA_DIR: path.resolve("artifacts/drop-data") },
    timeout: 60_000,
  },
});
