...
class Bridge:
    ...
    def probe(self, track: int, device: str, params: list | None = None):
        ...
        with self.lock:
            ...
            self.pending = {"id": uid(), "kind": "insert_device", "track": int(track), "device": str(device), "params": params or [], "created": time.time()}
            self.result = None
            return self.pending["id"]
